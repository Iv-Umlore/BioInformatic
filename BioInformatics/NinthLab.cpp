...
//////////////////////////////////////////////////////////////////////////////////////
//						9.3 String Reconstruction Problem
//////////////////////////////////////////////////////////////////////////////////////
...
int FindElementPositionOnVector(string elem, vector<string>& vect) {
...
map<int, vector<int>> GetGraph(vector<string>& vect) {
	...
	for (int i = 0; i < vect.size(); i++)
		for (int j = i + 1; j < vect.size(); j++) {
			if (getSubstring(vect[i], 1, kmerSize) == getSubstring(vect[j], 0, kmerSize))
				result[FindElementPositionOnVector(vect[i],vect)].push_back(FindElementPositionOnVector(vect[j],vect));

			if (getSubstring(vect[i], 0, kmerSize) == getSubstring(vect[j], 1, kmerSize))
				result[FindElementPositionOnVector(vect[j], vect)].push_back(FindElementPositionOnVector(vect[i], vect));
		}
...
map<int, vector<int>> GetInputMapa(map<string, vector<string>>& startMapa, vector<string>& kmers) {
	map<int, vector<int>> result;

	for (auto iter = startMapa.begin(); iter != startMapa.end(); iter++)
		for (int i = 0; i < iter->second.size(); i++)
			result[FindElementPositionOnVector(iter->first, kmers)].push_back(FindElementPositionOnVector(iter->second[i], kmers));
...
public:
	...
	// Печать
	void BuildPath(int startCircle, int startValue = -1) {
		...
		if (startCircle == _activeCircles[0]) {
			_path.push_back(_circles[startCircle][i]);
		}

		for (int j = 0; j < _circles.size(); j++) {
			if (!IsActiveCircle(j) && CircleHaveThis(j, _circles[startCircle][i]))
				BuildPath(j, _circles[startCircle][i]);
		}
		...
		if (startCircle == _activeCircles[0])
			_path.push_back(_circles[startCircle][_circles[startCircle].size()-1]);
		_circles[startCircle].clear();
...
// Для запуска на больших файлах легче использовать ввод и вывод в файлы
//	ifstream in;
//	in.open("input.txt");
//
//	ofstream out;
//	out.open("output.txt");
	...
	getline(cin, str1);
	kmerSize = GetNumber(str1, tmpPosition);

	while(getline(cin, str1))
		kmers.push_back(str1);
	...
	searcher.BuildPath(tmp, start);

	auto pathVect = searcher.GetPath();

	cout << kmers[pathVect[0]];
	for (int i = 1; i < pathVect.size(); i++) {
		cout << kmers[pathVect[i]][kmerSize-1];
	}

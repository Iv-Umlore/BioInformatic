...
// Работает
int get_maximum(int start, int vertical, int horizontal, int diagonal, char chr1, char chr2, VectorPath& vect) {

	if (horizontal >= vertical && horizontal > diagonal && horizontal > start) {
		vect = HORIZONTAL;
		return horizontal;
	}

	if (vertical >= horizontal && vertical > diagonal && vertical > start) {
		vect = VERTICAL;
		return vertical;
	}

	if (diagonal >= horizontal && diagonal >= vertical && diagonal >= start) {
		vect = DIAGONAL;
		return diagonal;
	}
...
void math_current_distance(wordPath** arr, string& str1, string& str2, const map<string,int>& penaltyMap) {
	...
	for (int i = 1; i < str1.length(); i++) {
		for (int j = 1; j < str2.length(); j++) {
			value = get_maximum(
				GetScore(str1[i], str2[j], penaltyMap),
				arr[i - 1][j].value + GetScore(str1[i], '-', penaltyMap),
				arr[i][j - 1].value + GetScore('-', str2[j], penaltyMap),
				arr[i - 1][j - 1].value + GetScore(str1[i], str2[j], penaltyMap),
				str1[i],
			...
			case DIAGONAL:
				...
				break;
			case START:
				arr[i][j].first = str1[i];
				arr[i][j].second = str2[j];
				break;
			default:
				break;
			}

			if (arr[i][j].value > bester.value)
				bester = arr[i][j];

		}
	}

	auto tmpWords = arr[str1.length() - 1][str2.length() - 1];
	arr[str1.length() - 1][str2.length() - 1] = (tmpWords.value < bester.value) ? bester : tmpWords;
}
...
int main() {
	...
	vector<string> tmpStringArray;
	map<string, int> penaltyMap;
	ifstream in("PAM250_1.txt");
	ifstream in2("dataset_216105_1.txt");
